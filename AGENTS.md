...
- For a direct search, the first ChatGPT Web request receives the current user prompt exactly. When the current prompt asks the local model to formulate the question first, or that model produces the first concrete `web_search` query, the first request instead sends an anchored envelope containing the current user prompt and that query, and instructs ChatGPT to correct mistaken names, versions, and premises in that query. Only when the user explicitly configures a limit of two or three may a later request add another individual `web_search` query as an unverified additional research angle anchored to the current user prompt and instruct ChatGPT to correct mistaken names, versions, and premises in that angle. Taceta does not directly send conversation history, system messages, attachments, or Thinking traces.
