...
OAuth proxyの互換性チェックには、実装の参照元であるGrok Build `1.0.24` を `x-grok-client-version` として送り、トークン交換にも同じ値を使います。この値は[参照commitの版定義](https://github.com/xai-org/grok-build/blob/37949780c144e37df692e3d669051a21fec24f20/crates/codegen/xai-grok-version/Cargo.toml)に対応し、[公開samplerが送るヘッダー](https://github.com/xai-org/grok-build/blob/37949780c144e37df692e3d669051a21fec24f20/crates/codegen/xai-grok-sampler/src/client.rs#L554-L608)に合わせています。接続ヘッダーは実働の `grok-codex-bridge` を再利用し、認証されたユーザーIDを `x-userid` と `x-grok-user-id` に、接続用の識別子を `grok-shell`、実行モードを `headless` として送ります。User-Agentと製品表示はTaceta自身を示します。ユーザーIDは公式OAuth userinfoから取得し、Taceta専用キーチェーンでトークンとともに保持します。互換性値はソース内で固定し、通信契約の対応なしに最新リリースへ追随させません。HTTP 426の表示は互換性更新の要求を伝え、Tacetaの利用禁止やアカウント拒否とは判定しません。
...
For the OAuth proxy's compatibility check and token exchange, requests send `x-grok-client-version: 1.0.24`, the Grok Build release used as this adapter's implementation reference. This follows the [reference commit's version declaration](https://github.com/xai-org/grok-build/blob/37949780c144e37df692e3d669051a21fec24f20/crates/codegen/xai-grok-version/Cargo.toml) and [public sampler header](https://github.com/xai-org/grok-build/blob/37949780c144e37df692e3d669051a21fec24f20/crates/codegen/xai-grok-sampler/src/client.rs#L554-L608). Connection headers reuse the working `grok-codex-bridge` contract: the authenticated user ID in both `x-userid` and `x-grok-user-id`, protocol identifier `grok-shell`, and mode `headless`. User-Agent and product presentation identify Taceta itself. The user ID comes from the official OAuth userinfo endpoint and is stored with tokens in Taceta's own Keychain entry. The compatibility value stays pinned in source and does not follow release updates without corresponding protocol support. HTTP 426 reports a compatibility upgrade requirement; it does not establish that Taceta or the account is prohibited.
