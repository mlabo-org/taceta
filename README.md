...
Grok／Ollamaで進めた作業からGPTへ切り替える場合は、同じTacetaの作業とフォルダーを引き継げます。「続行」または次の指示の送信で、新しいCodex会話に元の依頼・追加指示・決定事項・完了／未完了・現在の作業文脈と実行結果を一度だけ渡します。現在のファイルはそのまま使い、元の作業記録も保持します。以前の詳細が必要なときは、その作業だけの原文記録を参照できます。Thinkingや途中で切れた回答、以前の操作承認は引き継ぎません。結果不明の操作を再実行する指示にも変換しません。GPTからGrok／Ollamaへ戻す場合も、同じ作業とフォルダーを使います。Codexの保存済み会話を読み取り、追加指示・回答・ファイル変更・コマンド結果をTacetaの原文記録へ追記してから接続先を切り替えます。取り込み失敗やキャンセルではGPT側を維持します。以前の完了／未完了は、その後の実行結果と現在のコードを照合して続行します。再びGPTへ渡す場合は更新後の記録で新しいCodex会話を作り、古い会話へ黙って戻りません。通常チャットの接続先をまたぐ切り替えは、別の会話として保存します。
